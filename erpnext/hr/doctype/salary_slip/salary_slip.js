...
frappe.ui.form.on("Salary Slip", "employee", function(frm){
	...
	frappe.call({
		method: "erpnext.hr.doctype.salary_slip.salary_slip.get_leave_details",
		args: {
			employee: frm.doc.employee
			},
			callback: function(r){
					console.log(r.message)
					        ...
					        frappe.model.set_value(v.doctype, v.name, "leave_allocated", r.message[e].allocated_leave)
					        frappe.model.set_value(v.doctype, v.name, "leave_taken", r.message[e].leave_taken)
					        frappe.model.set_value(v.doctype, v.name, "leave_balance", r.message[e].leave_remaining)
